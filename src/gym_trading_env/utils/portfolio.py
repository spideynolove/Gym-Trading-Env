class Portfolio:
    ...
    def valorisation(self, price):
        return sum([self.asset * price, self.fiat, -self.interest_asset *
            price, -self.interest_fiat])
    def real_position(self, price):
        return (self.asset - self.interest_asset) * price / self.valorisation(
            price)
    ...
    def trade_to_position(self, position, price, trading_fees):
        ...
        if position <= 0 and current_position < 0:
            ...
            interest_reduction_ratio = min(1, (position - 1) / (
                current_position - 1))
        if interest_reduction_ratio < 1:
            self.asset = self.asset - (1 - interest_reduction_ratio
                ) * self.interest_asset
            self.fiat = self.fiat - (1 - interest_reduction_ratio
                ) * self.interest_fiat
            self.interest_asset = (interest_reduction_ratio * self.
                interest_asset)
    ...
    def describe(self, price):
        print('Value : ', self.valorisation(price), 'Position : ', self.
            position(price))
