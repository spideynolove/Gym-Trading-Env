...
def create_financial_chart(df: pd.DataFrame, lines: list = None):
    """
    Creates a multi-layer financial chart using pyecharts including:
    - Candlestick chart
    - Volume bar chart
    - Portfolio value
    - Position changes
    - Cumulative rewards
    - Optional custom indicator lines (e.g., moving averages)

    Args:
        df (pd.DataFrame): DataFrame with datetime index and columns:
                           ['open', 'high', 'low', 'close', 'volume', 'portfolio_valuation', 'position', 'reward']
        lines (list): List of dictionaries defining custom lines to overlay on the candlestick chart.
                      Each dict should have:
                          - 'name': str
                          - 'function': callable (df) -> pd.Series
                          - 'line_options': dict (optional) style options

    Returns:
        Grid: A pyecharts Grid object containing all sub-charts.
    """
    if lines is None:
        lines = []
    ...
    # Unique key prefix to avoid column name conflicts
    line_key_prefix = 'ievi4G3vG678Vszad'

    # Add custom indicator lines to the DataFrame
    for line in lines:
    ...
    # Define layout architecture (height and position in %)
    layout = {
        'candlesticks': {'height': 35, 'top': 10},
        'randle_slider': {'top': 2},  # Typo? Should be "slider"? But kept as-is.
    ...
    # Convert numeric values to percentage strings for layout
        ...
        .set_global_opts(
            ...
            datazoom_opts=[
                ...
                opts.DataZoomOpts(
                    ...
                    type_='slider',
                    xaxis_index=list(range(5)),
                    pos_top=layout['randle_slider']['top_%'],
                    range_start=95,
                    range_end=100
                )
            ],
    ...
    # Overlay custom lines (e.g., moving averages, signals)
    for line in lines:
        series_name = line['name']
        y_data = line['function'](df).tolist()
        line_style = opts.LineStyleOpts(**line['line_options']) if 'line_options' in line else None
    ...
    volume_chart = (
        ...
        .set_global_opts(
            ...
            title_opts=opts.TitleOpts(
                is_show=True,
                title="Volume",
                pos_top=f"{layout['volumes']['top'] - 1}%",
                pos_left="50%",
                text_align="center",
    ...
    portfolio_chart = (
        ...
        .set_global_opts(
            ...
            title_opts=opts.TitleOpts(
                ...
                pos_top=f"{layout['portfolios']['top'] - 3}%",
                pos_left="50%",
                text_align="center",
                title_textstyle_opts=opts.TextStyleOpts(
                    font_size='14px',
    ...
    positions_chart = (
        ...
        .set_series_opts(areastyle_opts=opts.AreaStyleOpts(opacity=0.2, color='blue'))
        .set_global_opts(
            xaxis_opts=opts.AxisOpts(
                axislabel_opts=opts.LabelOpts(is_show=False),
                axisline_opts=opts.AxisLineOpts(is_show=False),
            ...
            title_opts=opts.TitleOpts(
                is_show=True,
                title="Positions",
                pos_top=f"{layout['positions']['top'] - 3}%",
                pos_left="50%",
                text_align="center",
                title_textstyle_opts=opts.TextStyleOpts(
                    font_size='14px',
    ...
    rewards_chart = (
        ...
        .set_series_opts(areastyle_opts=opts.AreaStyleOpts(opacity=0.3, color='blue'))
        .set_global_opts(
            xaxis_opts=opts.AxisOpts(
                axislabel_opts=opts.LabelOpts(is_show=False),
                axisline_opts=opts.AxisLineOpts(is_show=False),
            ...
            title_opts=opts.TitleOpts(
                ...
                pos_top=f"{layout['rewards']['top'] - 3}%",
                pos_left="50%",
                text_align="center",
                title_textstyle_opts=opts.TextStyleOpts(
                    font_size='14px',
    ...
    # Add each subplot to the grid with proper positioning
    grid_chart.add(
        candlestick_chart,
        grid_opts=opts.GridOpts(
            pos_left='10%',
            pos_right='8%',
            pos_top=layout['candlesticks']['top_%'],
    ...
    grid_chart.add(
        ...
        grid_opts=opts.GridOpts(
            pos_left='10%',
            pos_right='8%',
            pos_top=layout['volumes']['top_%'],
            height=layout['volumes']['height_%']
        )
    )
    grid_chart.add(
        portfolio_chart,
        grid_opts=opts.GridOpts(
            pos_left='10%',
            pos_right='8%',
            pos_top=layout['portfolios']['top_%'],
            height=layout['portfolios']['height_%']
        )
    )
    grid_chart.add(
        positions_chart,
        grid_opts=opts.GridOpts(
            pos_left='10%',
            pos_right='8%',
            pos_top=layout['positions']['top_%'],
            height=layout['positions']['height_%']
        )
    )
    grid_chart.add(
        rewards_chart,
        grid_opts=opts.GridOpts(
            pos_left='10%',
            pos_right='8%',
            pos_top=layout['rewards']['top_%'],
            height=layout['rewards']['height_%']
        )
    )
