...
async def _ohlcv(exchange: ccxt.Exchange, symbol: str, timeframe: str, limit: int, step_since: int, timedelta: int) -> pd.DataFrame:
    """
    ...
    df = pd.DataFrame(result, columns=["timestamp_open", "open", "high", "low", "close", "volume"])
...
async def _download_symbol(
    ...
    """
    Download full history of OHLCV data for a single symbol with rate limit handling.
    """
    if since is None:
    ...
    for step_since in range(since, until, limit * timedelta_ms):
        tasks.append(_ohlcv(exchange, symbol, timeframe, limit, step_since, timedelta_ms))
    ...
    final_df = final_df[(final_df["timestamp_open"] > since) & (final_df["timestamp_open"] < until)]
...
async def _download_symbols(
    ...
    **kwargs
    ...
    """
    Download and save OHLCV data for multiple symbols from one exchange.
    """
    ...
    try:
        for symbol in symbols:
            ...
            filepath = f"{dir}/{exchange_name}-{symbol.replace('/', '')}-{timeframe}.pkl"
            df.to_pickle(filepath)
            print(f"{symbol} downloaded from {exchange_name} and stored at {filepath}")
    finally:
...
async def _download(
    ...
    """
    Download data from multiple exchanges concurrently.
    """
    if until is None:
...
def download(*args, **kwargs):
    """
    Synchronous wrapper for _download to run the async event loop.
    """
