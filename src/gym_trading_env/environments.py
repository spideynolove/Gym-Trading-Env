...
class TradingEnv(gym.Env):
    ...
    def __init__(self, df: pd.DataFrame, positions: List[Union[int, float]]
                 = None, dynamic_feature_functions: List[Callable[[History], float]] =
                 None, reward_function: Callable[[History], float] =
                 basic_reward_function, windows: Optional[int] = None, trading_fees:
                 float = 0.0, borrow_interest_rate: float = 0.0, portfolio_initial_value:
                 float = 1000.0, initial_position: Union[str, float] = 'random',
                 max_episode_duration: Union[int, str] = 'max', verbose: int = 1, name:
                 str = 'Stock', render_mode: Optional[str] = 'logs'):
        super().__init__()
        ...
        self.name = name
        assert self.initial_position in self.positions or self.initial_position == 'random', "Initial position must be 'random' or in positions list."
        ...
        obs_shape = (self._nb_features,) if self.windows is None else (self
                                                                       .windows, self._nb_features)
        self.observation_space = spaces.Box(-np.inf,
                                            np.inf, shape=obs_shape, dtype=np.float32)

    def _set_df(self, df: pd.DataFrame) -> None:
        df = df.copy()
        self._features_columns = [col for col in df.columns if 'feature' in col
                                  ]
        self._info_columns = list(set(df.columns) | {'close'} - set(self.
                                                                    _features_columns))
    ...
    def _get_obs(self) -> np.ndarray:
        for i, func in enumerate(self.dynamic_feature_functions):
            self._obs_array[self._idx, self._nb_static_features + i] = func(
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[dict] = None,
        ...
        if isinstance(self.max_episode_duration, int):
            ...
            self._idx = np.random.randint(low=max(low, 0), high=max(high,
                                                                    low + 1))
        price = self._get_price()
        self._portfolio = TargetPortfolio(
            position=self._position, value=self.portfolio_initial_value, price=price)
    ...
    def _take_action_order_limit(self) -> None:
        ...
        for position, params in self._limit_orders.items():
            if position != self._position and params['limit'
                                                     ] <= high and params['limit'] >= low:
    ...
    def add_limit_order(self, position: float, limit: float, persistent:
                        bool = False) -> None:
        self._limit_orders[position] = {'limit': limit, 'persistent':
                                        persistent}

    def step(self, action: Optional[int] = None) -> tuple[np.ndarray, float,
                                                          bool, bool, dict]:
        ...
        truncated = self._idx >= len(self.df) - 1 or isinstance(self.
                                                                max_episode_duration, int
                                                                ) and self._step >= self.max_episode_duration - 1
        self.historical_info.add(idx=self._idx, step=self._step, date=self.
                                 df.index.values[self._idx], position_index=action, position=self._position, real_position=self._portfolio.real_position(
                                     price), data=dict(zip(self._info_columns, self._info_array[self
                                                                                                ._idx])), portfolio_valuation=portfolio_value,
        ...
        if done or truncated:
            ...
            self.log()
        return self._get_obs(), reward, done, truncated, self.historical_info[
            -1]
    ...
    def calculate_metrics(self) -> None:
        close_returns = self.historical_info['data_close', -1
                                             ] / self.historical_info['data_close', 0] - 1
        portfolio_returns = self.historical_info['portfolio_valuation', -1
                                                 ] / self.historical_info['portfolio_valuation', 0] - 1
        self.results_metrics = {'Market Return':
                                f'{100 * close_returns:5.2f}%', 'Portfolio Return':
                                f'{100 * portfolio_returns:5.2f}%'}
    ...
    def log(self) -> None:
        if self.verbose > 0:
            text = ' | '.join(f'{k}: {v}' for k, v in self.results_metrics.
                              items())
            print(text)
    ...
    def save_for_render(self, dir_path: str = 'render_logs') -> None:
        ...
        columns = [col for col in self.historical_info.columns if not col.
                   startswith('date_')]
        history_df = pd.DataFrame([self.historical_info[c] for c in columns
                                   ], columns=columns)
        ...
        filepath = (
            f'{dir_path}/{self.name}_{datetime.datetime.now():%Y-%m-%d_%H-%M-%S}.pkl'
        )
        render_df.to_pickle(filepath)


class MultiDatasetTradingEnv(TradingEnv):
    def __init__(self, dataset_dir: str, *args, preprocess: Callable[[pd.
                                                                      DataFrame], pd.DataFrame] = lambda df: df,
                 episodes_between_dataset_switch: int = 1, **kwargs):
        ...
        if not self.dataset_pathes:
            raise FileNotFoundError(
                f'No datasets found with pattern: {dataset_dir}')
