...
class TestSessionManager(unittest.TestCase):
    ...
    def test_london_ny_overlap_detection(self):
        ...
        dt_no_overlap = datetime.datetime(2024, 1, 15, 10, 0, 0, tzinfo=
            pytz.UTC)
        self.assertTrue(self.session_manager.is_london_ny_overlap(dt_overlap))
        self.assertFalse(self.session_manager.is_london_ny_overlap(
            dt_no_overlap))
    ...
    def test_liquidity_score_single_session(self):
        dt_london_only = datetime.datetime(2024, 1, 15, 10, 0, 0, tzinfo=
            pytz.UTC)
    ...
    def test_volatility_multiplier_single_session(self):
        dt_london_only = datetime.datetime(2024, 1, 15, 10, 0, 0, tzinfo=
            pytz.UTC)
        volatility = self.session_manager.get_volatility_multiplier(
            dt_london_only)
    ...
    def test_active_currency_pairs_london_ny_overlap(self):
        ...
        expected_pairs = {'EUR/USD', 'GBP/USD', 'USD/CHF', 'EUR/GBP',
            'EUR/CHF', 'GBP/CHF', 'USD/CAD', 'USD/JPY'}
        self.assertTrue(expected_pairs.issubset(pairs))
    def test_holiday_detection_uk(self):
        dt_uk_holiday = datetime.datetime(2024, 12, 25, 12, 0, 0, tzinfo=
            pytz.UTC)
    ...
    def test_holiday_adjusted_liquidity_both_holidays(self):
        dt_both_holidays = datetime.datetime(2024, 1, 1, 15, 0, 0, tzinfo=
            pytz.UTC)
        adjusted_liquidity = (self.session_manager.
            get_holiday_adjusted_liquidity(dt_both_holidays))
    ...
    def test_should_avoid_trading_both_holidays(self):
        dt_both_holidays = datetime.datetime(2024, 1, 1, 15, 0, 0, tzinfo=
            pytz.UTC)
        should_avoid = self.session_manager.should_avoid_trading(
            dt_both_holidays)
        self.assertTrue(should_avoid)
    def test_should_avoid_trading_late_night(self):
        dt_late_night = datetime.datetime(2024, 1, 15, 23, 0, 0, tzinfo=
            pytz.UTC)
    ...
    def test_session_info_completeness(self):
        ...
        required_keys = ['timestamp', 'active_sessions', 'session_overlaps',
            'is_london_ny_overlap', 'liquidity_score',
            'volatility_multiplier', 'active_currency_pairs',
